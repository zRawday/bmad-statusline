...
- 2026-06-10 — Code-review correction: `persistUsageSnapshot` now uses a per-process temp file (`weekly-usage.json.<pid>.tmp`) with unlink-on-failure, replacing the shared `.tmp` that could tear under concurrent renders (review finding, contradicted AC7). Tests `(c)`/`(f)` updated accordingly. Full suite still green (707 pass / 0 fail). Architecture.md reference body + AC7 wording to be reconciled in story 10.5.

## Review Findings

**Code review 2026-06-10** (Blind Hunter + Edge Case Hunter + Acceptance Auditor). Acceptance Auditor verdict: **PASS** — all 10 ACs fully satisfied, verbatim-conformant to the Rev.7 reference bodies, no anti-patterns committed, scope clean (reader + test + this story-doc only), 707/707 tests green. Outcome: **1 finding patched (on Fred's request), 1 deferred, 4 dismissed.**

### Patched (review correction — Fred authorised fixing now rather than deferring)

- [x] **[Review][Patch] Shared fixed `.tmp` filename defeats write-atomicity under concurrent renders** [`src/reader/bmad-sl-reader.js` `persistUsageSnapshot`] — `persistUsageSnapshot` originally wrote to a single shared `weekly-usage.json.tmp` (not per-pid). Up to 3 concurrent `line N` readers + the standalone reader raced on the same temp filename: interleaved `writeFileSync` could publish a torn snapshot, and the loser's `renameSync` threw ENOENT (swallowed). The Edge Case Hunter **empirically reproduced** the ENOENT cross-rename on this Windows host. This contradicted **AC7**'s claim that concurrent writes are torn-safe and "a torn read is impossible." **Fix applied:** per-process temp name `USAGE_PATH + '.' + process.pid + '.tmp'` (account-global snapshot needs pid scoping — unlike the hook's naturally-unique session-scoped `status-<sid>.json.tmp`); each writer now owns a private temp and only the atomic rename is shared, so torn writes and cross-rename ENOENT are eliminated. The temp is `unlink`'d on write/rename failure so a failed attempt leaves no orphan. This restores AC7's guarantee in fact. Tests: `(c)` now also asserts no `.tmp` leftover (rename consumed it); `(f)` reworked to force the error via the final path (pid-independent) and assert no per-pid `.tmp` orphan. Full suite **707/707 green**. **NOTE for story 10.5:** update the Rev.7 reference body in `architecture.md` (and AC7's wording) to show the per-pid temp, so the locked spec matches the corrected implementation. The original verbatim reference is now intentionally superseded by this reviewed correction.
...
_(Originally-deferred ".tmp orphans never purged" is now largely addressed by the unlink-on-failure in the patch above; the only residual is a hard-crash orphan between write and rename, which is bounded by pid reuse and consistent with the hook's own un-swept `status-<sid>.json.tmp` — no separate action needed.)_
