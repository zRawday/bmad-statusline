...
- **Shared fixed `.tmp` filename defeats write-atomicity under concurrent renders** (Medium, latent) — `persistUsageSnapshot` in `src/reader/bmad-sl-reader.js` writes to a single shared `weekly-usage.json.tmp` instead of a per-pid temp. Up to 3 concurrent `line N` readers + the standalone reader race on the same temp filename: interleaved `writeFileSync` can publish a torn snapshot, and the loser's `renameSync` throws ENOENT (silently swallowed). Empirically reproduced on Windows during review. Contradicts AC7's "a torn read is impossible" claim. Latent until story 10.4 (TUI screen) actually reads `weekly-usage.json`. **Fix (do in 10.5, the Rev.7 reconciliation story):** use `const tmp = USAGE_PATH + '.' + process.pid + '.tmp';` then `writeFileSync(tmp, …); renameSync(tmp, USAGE_PATH);` — mirrors the hook's session-scoped `status-<sid>.json.tmp`. Also correct AC7's over-stated safety wording in `architecture.md`. The dev correctly implemented the locked verbatim reference; this is a spec/architecture hardening, deferred per the project convention (cf. the 10.1-deferred `resets_at` finding) rather than patched into the consumer story.
- **`weekly-usage.json.tmp` orphans never purged** (Low) — if a reader dies between write and rename, the `.tmp` lingers; neither `purgeStale` (`src/reader/bmad-sl-reader.js`, matches `.alive-*`) nor `src/clean.js` (matches `status-*.json`) removes it. Cosmetic with the current single fixed name (overwritten next run); becomes material once the per-pid temp fix above lands (one orphan per crashed pid). Bundle the `*.tmp` cleanup with the fix above in 10.5.
