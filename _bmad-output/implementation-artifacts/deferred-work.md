...
- **Doc reconciliation for the per-pid temp fix** (housekeeping, do in 10.5) — story 10.2's review replaced the shared `weekly-usage.json.tmp` with a per-process `weekly-usage.json.<pid>.tmp` in `persistUsageSnapshot`. The Rev.7 reference body in `_bmad-output/planning-artifacts/architecture.md` and AC7's "no extra locking / torn read impossible" wording still describe the old shared-`.tmp` form. Update them in 10.5 so the locked spec matches the corrected implementation.
- **Self-color exclusion is a growing `!==` chain** (Low, pre-existing) — `src/reader/bmad-sl-reader.js:276` chains `widgetId !== 'bmad-llmstate' && !== 'bmad-contextpct' && !== 'bmad-weeklyusage'`. Refactor to `SELF_COLORED_WIDGETS = new Set([...])` and test membership, so the next self-coloring widget doesn't require editing this exact condition. Pre-existing pattern (predates story 10.2); maintainability cleanup for a future pass.

## Deferred from: review of spec-doctor-health-check (2026-05-29)

- **`defaultRunStatusline` does not drain child stdout** — `spawn` is called with default stdio so the child's stdout is piped but never consumed. If `ccstatusline` ever writes more than the OS pipe buffer (~64KB) to stdout, it blocks and `doctor` stalls to the 60s timeout, then wrongly reports a timeout failure and purges a healthy cache. Current ccstatusline output is a single short line, so risk is theoretical. Fix: set `stdio: ['pipe', 'ignore', 'pipe']`.
- **Windows timeout kills the shell, not npx** — on win32 the functional check spawns `npx -y ccstatusline@latest` via `shell: true`; on the 60s timeout `child.kill()` terminates `cmd.exe` but can orphan the `npx`/`node` grandchild. Fix needs `detached: true` + `process.kill(-child.pid)`. Low impact (timeout rarely fires — npx either works or fails fast).
- **`HealthCheckScreen` rapid re-run can clobber state** — pressing `r` calls `runChecks()` directly and discards its cancellation cleanup (only the mount effect's cleanup is wired). If a user spams `r` faster than checks resolve, an earlier slow promise can resolve after a newer one and overwrite state, or call setState after unmount. Cosmetic; only under rapid re-run. Fix: track latest-run id in a ref.
- **`purgeCcstatuslineNpxCache` swallows `rmSync` failures** — if a matching ccstatusline cache dir is found but deletion throws (e.g. a file locked by the running ccstatusline process on Windows), the catch swallows it and the entry is not counted, so `purged.length` stays 0 and the user sees the misleading "no ccstatusline cache to purge" message instead of a lock/permission hint. Fix: track delete failures separately and surface them.
- **No coverage for the real `defaultRunStatusline`** — every doctor test injects a fake runner, so the only OS-risky code (spawn, win32 shell quoting, pipe draining, EPIPE, timeout) has zero direct tests. Hard to test without a real/mocked child process; the design deliberately injects the runner.
- **Pre-existing: `install.js` `defaultPaths` ignores `BMAD_CONFIG_DIR`** — install writes the reader to a hardcoded `~/.config/bmad-statusline` while the runtime reader/hook/config-loader resolve `process.env.BMAD_CONFIG_DIR || …`. If a user sets `BMAD_CONFIG_DIR`, install deploys to the wrong dir vs where the reader reads. Not caused by this story (doctor was patched to honor the env var, exposing the inconsistency). Fix install + uninstall `defaultPaths` to honor `BMAD_CONFIG_DIR`.
...
## Deferred from: code review of 10-1-shared-weekly-usage-computation-esm-bridge-re-exports (2026-06-10)

- **`resets_at` not type/finite-guarded in `computeWeeklyUsage` (asymmetric with `used_percentage`)** — a non-numeric `resets_at` (string `"abc"`, `[]`, `{}`) bypasses the `== null` guard, makes `weekStartMs`/`timePct` `NaN`, and the zone ladder falls through to `slowdown` → a spurious **SLOW DOWN / red** for healthy usage. NOT a defect vs Story 10.1's contract: AC7 type-guards `used_percentage` only, and the VERIFIED data source (`rate_limits.seven_day.resets_at`, seconds) always supplies a number; patching would diverge from the copy-verbatim Architecture Rev.7 reference. Harden in the consumer that builds the snapshot (story 10.2 reader) and/or 10.5 Rev.7 reconciliation — mirror the usagePct guard: `if (typeof snapshot.resets_at !== 'number' || !isFinite(snapshot.resets_at)) return null;`. Raised High by Blind Hunter + Edge Case Hunter. [src/reader/shared-constants.cjs:98]
- **Week-end `slowdown` unreachable** — once `timePct` clamps to 100 (now ≥ reset), the `slowdown` branch requires `usagePct ≥ 110`, impossible for a ≤100 percentage, so a maxed-out user at the very end of the week renders **TOO HIGH** (high), never **SLOW DOWN**. Property of the LOCKED Architecture Rev.7 zone formula, faithfully implemented here; changing it is an architecture/product decision. Flag for story 10.5 (`rev7-flag-reconciliation … intentional-asymmetry`). Raised Medium by Blind Hunter. [src/reader/shared-constants.cjs:105-108]

## Deferred from: code review of non-bmad-session-support (2026-08-09)

- **Status files are never auto-reaped** — `purgeStale()` deletes only `.alive-*`; `status-*.json` orphans are cleaned opportunistically on TUI launch (`cleanOrphanedStatusFiles`) or via `clean`. With every session now tracked, the cache grows on machines that rarely open the TUI. Spec accepted this ("Ask First" bullet). Fix: unlink `status-<sid>.json` in `purgeStale()` when its mtime exceeds `ALIVE_MAX_AGE_MS`.
- **`MAX_SESSIONS = 20` keeps the OLDEST sessions** — `pollSessions` sorts ascending by `started_at` then `slice(0, 20)`, so past 20 live sessions a newly started one never appears. Pre-existing; reachable now that non-BMAD sessions count toward the cap. Sorting unanchored sessions last mitigates but does not fix it.
- **Non-BMAD tabs all render white** — `sessionLabel` falls back to an 8-char session id, which never matches the skill catalog, so `resolveSessionColor` returns `'white'` for every non-BMAD tab. Consider `hashProjectColor(sessionId)` for the no-workflow case.
- **`project` is a bare basename, so distinct directories collide** — `~/work/api` and `~/personal/api` merge into one monitor group. Pre-existing (same fallback already applied to BMAD projects without `config.yaml`), but far more reachable now. Fix would be a status-schema change: store `project_path`, group by it, label by basename.
- **Matcher migration only touches `~/.claude/settings.json`** — a legacy bmad matcher in project-scope `settings.json` / `settings.local.json` is not widened and can coexist with the new global wildcard entry, double-firing the hook.

## Deferred from: review of spec-restore-auto-allow (2026-08-15)

- **`.autoallow-{sid}` flag files are never reaped** — the early SessionEnd block deletes only `.alive-{sid}`, and `purgeStale()` matches only `.alive-*`. A per-session auto-allow flag therefore outlives its session forever. Harmless (session ids are unique, so a stale flag can never re-enable a different session) but the cache grows without bound. Fix: unlink `.autoallow-<sid>` alongside `.alive-<sid>` in the SessionEnd block, and match it in `purgeStale()`.
- **The allowlist is duplicated in five places with no drift detection** — `AUTO_ALLOW_TOOLS` in the hook, the `AutoAllowMenu` banner text, `README.md`, `_bmad-output/project-context.md`, and two test files each hardcode the tool list. `project-context.md` declares banner drift a defect ("a banner that overstates what happens is a defect") but nothing detects it. The hook is a self-executing standalone CJS script so the constant cannot be imported; a regex extraction from source — the technique the existing `emits allow only` guard test already uses — is the available option.
- **README omits the off-switch mechanics** — for an unattended-execution feature it documents neither where the per-session flag lives (`.autoallow-{sid}` in the cache dir) nor how to disable auto-allow without launching the TUI. A kill switch reachable only through an interactive menu is worth documenting as a file path too.
- **`PermissionRequest` does the full walk-up before deciding** — the `_bmad/` walk-up, project detection and a `writeStatus` all run before `handlePermissionRequest` is reached, adding filesystem work to the latency-sensitive interactive path. Pre-existing dispatch structure, not caused by this change; only now does it sit in front of a decision the user waits on. Related: `if (!cwd) process.exit(0)` means a payload without `cwd` never reaches the handler at all — a case the I/O matrix does not list.
- **`test/tui-app.test.js` has pre-existing CRLF line endings** while `.gitattributes` forces LF. Left alone deliberately so a focused change did not carry a 290-line whitespace diff. Normalize in a dedicated commit.
