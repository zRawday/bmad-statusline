...
- **`findClaudeAncestorPid()` leaks `wmic` stderr** — `execSync('wmic ...')` at `src/hook/bmad-hook.js:844` and `src/reader/bmad-sl-reader.js:120` inherits the child's stderr; the try/catch swallows the exception, not the stream. `wmic` was removed in Windows 11 26200, so every invocation prints "'wmic' n'est pas reconnu" — a Pattern-1 (silent-always) violation. Pre-existing, now firing in every session rather than only BMAD ones. Fix: `stdio: ['ignore', 'pipe', 'ignore']`, and prefer a PowerShell/`process.ppid` walk over `wmic`.
...
- **`monitor-utils.js` re-parses every status file every 1500 ms** — full read + `JSON.stringify` comparison on the Ink thread; degrades once sessions carry long `reads`/`writes`/`commands` histories. Fix: skip the re-read when `statSync(statusPath).mtimeMs` is unchanged.
