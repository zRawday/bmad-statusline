...
<step n="1" goal="Determine target story">
  ...
  <check if="no user input provided">
    ...
    <check if="this is first story in epic {{epic_num}}">
      ...
      <action>Commit epic status change: `git add {{sprint_status}}` then `git commit -m "update sprint-status: epic-{{epic_num}} → in-progress"`</action>
    </check>
  ...
  <check if="sprint status file exists at {{repo_root}}/{{sprint_status}}">
    ...
    <check if="commit fails with index.lock error">
      ...
      <check if="all retries fail">
        <output>⚠️ Sprint-status update failed (concurrent lock). Story spec is committed on branch. Update sprint-status manually.</output>
      </check>
    </check>
  </check>
