...
3. Merge to `{default_branch}` and cleanup worktree. Derive `{slug}` from `{spec_file}` filename (strip `spec-` prefix and `.md` suffix). `cd {repo_root}`. Run `git checkout {default_branch}` then `git merge {slug}`. If merge succeeds, remove worktree (`git worktree remove {worktree_path}`) and delete the branch (`git branch -d {slug}`). If merge conflicts, HALT — list conflicting files and ask the user to resolve manually. Do not force-merge. Do not remove worktree or branch. Indicate cleanup steps: "After resolution: `git worktree remove {worktree_path}` then `git branch -d {slug}`."
4. Open the spec in the user's editor so they can click through the Suggested Review Order:
...
5. Display summary of your work to the user, including the commit hash if one was created. Any file paths shown in conversation/terminal output must use CWD-relative format (no leading `/`) with `:line` notation (e.g., `src/path/file.ts:42`) for terminal clickability — the goal is to make paths clickable in terminal emulators. Include:
