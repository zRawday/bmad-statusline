...
<workflow>
      ...
      <check if="current status == 'ready-for-dev' OR review_continuation == true">
        ...
        <check if="commit fails with index.lock error">
          ...
          <check if="all retries fail">
            <output>⚠️ Sprint-status commit failed (concurrent lock). Continuing — update manually if needed.</output>
          </check>
        </check>
  ...
  <step n="10" goal="Commit story changes">
    ...
    <action>Stage changed files by listing them explicitly from `git status --porcelain` output (NOT `git add -A`)</action>
    <action>Extract story title from the story file</action>
    <action>Build commit message:
      - Title: `{{story_key}}: {{story_title}}`
      - Body (if review_continuation == true): "Includes code review follow-up fixes."
    </action>
    <action>Create commit with the constructed message</action>
    <output>Committed: {{story_key}}: {{story_title}}</output>
  </step>
